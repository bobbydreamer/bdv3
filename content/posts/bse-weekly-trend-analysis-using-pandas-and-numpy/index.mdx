...
Faced bit of a problem installing line_profiler in my system(Win10), so had to do a workaround, thats another topic, [here](../python-profiling). 
...
### # Things to do next
1. **Going forward, should i write only numpy programs ?**    
    It depends, to a certain extent. For large processing(laptop level), yes. Also, got to remember with too complex codes, maintainence will hit hard. Pandas are easy to understand than numpy. 
1. If any big program is running slow, profiling is the next thing you should do. 

### # Related articles
1. [Running python line_profiler in cloud shell](../python-profiling)
